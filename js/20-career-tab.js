...
const CAREER_CHOICES = {
  ...
  madsci: (p)=>[
    ...
    ...(!hasFlag(p,'transfusionResearch') ? [O('🩸','Research power transfusion','unlock a way to pass powers',0,()=>{ closeSheet(); if(p.stats.smarts>62||chance(50)){ setFlag(p,'transfusionResearch'); log(`${p.first} cracked it: a blood transfusion that can transfer superpowers into another body. Forbidden science — but it works.`,'big'); } else { p.stats.health=clamp(p.stats.health-8); log(`${p.first}'s transfusion trials failed — and nearly killed a test subject. Back to the lab.`,'bad'); } render(); })] : []),
    ...(hasFlag(p,'transfusionResearch') && (p.powers&&p.powers.length>0) && p.rels.some(r=>r.kind==='Child'&&r.alive && (findP(G,r.id)||{}).adopted) ? [O('🧬','Transfuse powers to adopted heir','passes ~2/3 of your powers',0,()=>{ closeSheet(); doPowerTransfusion(p); })] : []),
  ],
  proathlete: (p)=>[
