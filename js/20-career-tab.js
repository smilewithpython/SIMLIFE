...
const CAREER_CHOICES = {
  ...
  madsci: (p)=>[
    ...
    ...(!G.chromosomalMixing ? [O('🧪','Research chromosomal mixing','biological children for all couples',0,()=>{ closeSheet(); if(p.stats.smarts>65||chance(45)){ G.chromosomalMixing=true; log(`${p.first} achieved the impossible: a method of combining chromosomal material from any two people into a viable embryo. Same-sex couples in this bloodline can now have biological children — with full power inheritance. The breakthrough is permanent.`,'big'); } else { p.stats.health=clamp(p.stats.health-6); log(`${p.first}'s chromosomal research hit a dead end. The science isn't there yet.`,'bad'); } render(); })] : []),
  ],
  proathlete: (p)=>[
